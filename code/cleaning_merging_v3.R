...
excel_sheets(paste(data_in,"US_EX_0018_WITS.xlsx", sep= "/"))

EX_US <- read_excel(paste(data_in,"US_EX_0018_WITS.xlsx", sep= "/"), sheet = 2)
...
names(EX_US_l) <- c("reporter", "partner", "tradefl", "product", "indic", "yr",
                    "IM_fromUS")
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
# import CHN IM data 
excel_sheets(paste(data_in,"China_IM_9218_WITS.xlsx", sep= "/"))

IM_CHN <- read_excel(paste(data_in,"China_IM_9218_WITS.xlsx", sep= "/"), sheet = 2)

colnames(IM_CHN)


IM_CHN_l <- IM_CHN %>% 
  subset(select = c(year_col, "Reporter Name", "Partner Name", "Trade Flow",
...
names(IM_CHN_l) <- c("reporter", "partner", "tradefl", "product", "indic", "yr",
                    "EX_toCHN")
...
# attach any values to individual countries in this group)
IM_CHN_l <- IM_CHN_l %>%
  filter((partner != "Free Zones") & (partner != "Other Asia, nes") &
           (partner != "Unspecified") & (partner != "Czechoslovakia") &
           (partner != "Belgium-Luxembourg"))

#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
# import CHN EX data 
excel_sheets(paste(data_in,"China_EX_0018_WITS.xlsx", sep= "/"))

EX_CHN <- read_excel(paste(data_in,"China_EX_0018_WITS.xlsx", sep= "/"), sheet = 2)
...
names(EX_CHN_l) <- c("reporter", "partner", "tradefl", "product", "indic", "yr",
                     "IM_fromCHN")
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
# import GDP data to divide

library(wbstats)

my_wb_indicators <- c(gdp_cusd = "NY.GDP.MKTP.CD")

gdp_cddata <- wb_data(my_wb_indicators, start_date = 2010, end_date = 2020)

gdp_cddata <- gdp_cddata %>%
  subset(select = c("iso3c", "country", "date", "gdp_cusd"))

names(gdp_cddata) <- c("iso3code", "country", "yr", "gdpcusd")
...
trade_US_CHN <- merge(x = trade_US_CHN, y = gdp_cddata,
                      by.x = c("iso3code", "yr"),
                      by.y=c("iso3code", "yr"),
                      all.x = TRUE)

# prepare measure by calculating: (EX-IM)/GDP for both US and China separately
trade_US_CHN_f <- trade_US_CHN %>%
  mutate(ec_prox_US = (EX_toUS-IM_fromUS)/gdpcusd) %>%
  mutate(ec_prox_CHN = (EX_toCHN-IM_fromCHN)/gdpcusd) %>%
  subset(select = c(iso3code, yr, partner, ec_prox_US, ec_prox_CHN))

#save datafiles
write.csv(trade_US_CHN, paste(data_out, 'trade_US_CHN_merged.csv', sep = "/"), row.names = FALSE)
