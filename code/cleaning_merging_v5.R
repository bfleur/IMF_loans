...
########################################################################
# 2.6 IMF text reports (conditionality) data
...
articleiv_corpus_meta <- load((paste(data_in,"articleiv_corpus_meta.Rda", sep= "/")))

# we need word /token count, and sentence count
colnames(articleiv_corpus_meta)
str(articleiv_corpus_meta)

# Article IV docs

corpus_meta_cond_A <- articleiv_corpus_meta %>%
  filter(type %in% c("Article IV")) %>%
  filter(year_p > 2010 & year_p < 2020) %>%
  subset(select = -c(Types, year_s, date_pub, series, title, country))

library(reshape2)

cm_wide_token_A <- corpus_meta_cond_A %>%
  group_by(ccode) %>%
  mutate(row = row_number()) %>%
  select(-row)  %>%
  spread(type, Tokens) %>%
  #spread(type, Sentences) %>%
  ungroup()

names(cm_wide_token_A) <- c("text", "sent", "ccode", "year", "tokAIV")

cm_wide_sent_A <- corpus_meta_cond_A %>%
  group_by(ccode) %>%
  mutate(row = row_number()) %>%
  select(-row)  %>%
  spread(type, Sentences) %>%
...
names(cm_wide_sent_A) <- c("text", "token", "ccode", "year", "sentAIV")

cm_tokenA <- cm_wide_token_A %>%
  group_by(year, ccode) %>%
  summarise(sumtok_AIV = sum(tokAIV))

cm_sentA <- cm_wide_sent_A %>%
  group_by(year, ccode) %>%
  summarise(sumsent_AIV = sum(sentAIV))

# IMF Program docs
corpus_meta_cond_P <- articleiv_corpus_meta %>%
  filter(type == "IMF program document") %>%
  filter(year_p > 2010 & year_p < 2020) %>%
  subset(select = -c(Types, year_s, date_pub, series, title, country))

cm_wide_token_P <- corpus_meta_cond_P %>%
  group_by(ccode) %>%
  mutate(row = row_number()) %>%
  select(-row)  %>%
  spread(type, Tokens) %>%
...
names(cm_wide_token_P) <- c("text", "sent", "ccode", "year", "tokP")

cm_wide_sent_P <- corpus_meta_cond_P %>%
  group_by(ccode) %>%
  mutate(row = row_number()) %>%
  select(-row)  %>%
  spread(type, Sentences) %>%
...
names(cm_wide_sent_P) <- c("text", "token", "ccode", "year", "sentP")

#summarize the docs by year (shall we use mean or sum per year of there are more docs?)
cm_tokenP <- cm_wide_token_P %>%
  group_by(year, ccode) %>%
  summarise(sumstok_P = sum(tokP))


cm_sentP <- cm_wide_sent_P %>%
  group_by(year, ccode) %>%
  summarise(sumssent_P = sum(sentP))

# merge everything together
cm_m1 <- merge(x = cm_tokenA, y = cm_sentA,
                     by.x = c("ccode", "year"),
                     by.y=c("ccode", "year"),
                     all = TRUE)

cm_m2 <- merge(x = cm_m1, y = cm_tokenP,
               by.x = c("ccode", "year"),
               by.y=c("ccode", "year"),
               all = TRUE)

cm_m3 <- merge(x = cm_m2, y = cm_sentP,
               by.x = c("ccode", "year"),
               by.y=c("ccode", "year"),
               all = TRUE)
...
# save data file
write.csv(cm_m3, paste(data_out, 'corpus_meta_cond.csv', sep = "/"), row.names = FALSE)
